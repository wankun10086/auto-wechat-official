...
def main():
    ...
    if command == "generate":
        ...
        if len(sys.argv) < 3:
            print("请提供文章ID，例如: python main.py publish 1")
            return
        asyncio.run(cmd_publish(int(sys.argv[2])))
    elif command == "full":
...
async def cmd_publish(article_id=None):
    from src.scheduler.job_runner import ArticlePipeline
    pipeline = ArticlePipeline()
    result = await pipeline.create_draft(article_id)
    if result:
        print(f"草稿创建成功: {result.media_id or '已存在'}")
    else:
        print(f"草稿创建失败: {getattr(result, 'message', '未知错误')}")
...
async def cmd_full(strategy):
    ...
    if result:
        ...
        print(f"  状态: {result.status}")
        print("文章已进入草稿流程，请在微信公众号后台确认后手动发布。")
    else:
        print("流程失败")
